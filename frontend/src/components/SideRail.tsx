...
export default function SideRail({ activeLabel = 'library' }: SideRailProps) {
  ...
  return (
    <aside className="fixed left-0 top-0 z-20 flex h-screen w-12 flex-col sm:w-14 md:w-16">
      <button
        type="button"
        onClick={() => navigate('/dashboard')}
        className={`flex h-1/2 w-full items-center justify-center border-2 text-center text-sm font-semibold uppercase tracking-[0.2em] shadow-md ${activeLabel === 'dashboard'
      ...
      <button
        ...
        className={`flex h-1/2 w-full items-center justify-center border-2 text-center text-sm font-semibold uppercase tracking-[0.2em] shadow-md ${activeLabel === 'library'
